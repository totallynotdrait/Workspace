...
import datetime, programs, widgets, sys, psutil, threading
        ...
        with dpg.window(label="Control panel", height=750, show=False, width=1200, tag="control_panel", on_close=lambda:(dpg.hide_item("control_panel"), dpg.hide_item("theme"), dpg.hide_item("viewport"))):
            with dpg.group(horizontal=True):
                ...
                with dpg.child_window(height=-1, width=-1, horizontal_scrollbar=True, tag="viewport", show=False):
                    ...
                    dpg.add_color_picker(default_value=BACKGROUND_COLOUR, label="Clear color", tag="cp_cp", width=250, height=250, callback=color_picker_callback)
...
class ws_programs():
            ...
            with dpg.menu_bar():
                ...
                with dpg.menu(label="Programs"):
                    with dpg.menu(label="Workspace programs"):
                        ...
                        dpg.add_menu_item(label="XTerminal", callback=lambda:dpg.show_item("xterm"))
    ...
    def load_ws():
        ...
        def change_lws_status(text):
            dpg.configure_item("lws_status", label=text)
        ...
        with dpg.window(label="lws", width=900, height=300, tag="load_ws", no_title_bar=True, no_move=True) as lws:
            center_window(lws)
            center_window(lws)
            center_window(lws)
            ...
            with dpg.table(header_row=False):
                ...
                with dpg.table_row():
                    dpg.add_spacer()                 
                    dpg.add_button(label="None\n\n\n\n\n", width=-1, tag="lws_status")
                    dpg.bind_item_theme(dpg.last_item(), blank_button_theme)   
                    dpg.add_spacer()  
                with dpg.table_row():
                    dpg.add_spacer()                 
                    dpg.add_progress_bar(default_value=prog_status, width=-1, tag="lws_prog_status") 
                    dpg.add_spacer()
